    ...
    public class OnlineAppProcessingBL
    {
        ...
        public string ApplyPackageCheckOnlineDebitEBT(int PID, bool chkEBT, bool chkOnlineDebit)
        {
            ...
            if (dsPackInfo.Tables[0].Rows.Count > 0)
            {
                ...
                iRetVal = UpdateOtherProcessingBillingMethod(AppId, Convert.ToString(dr["billingMethod"]), Convert.ToBoolean(dr["Assessments"], dr["RollingReserve"].ToString().Trim()));
        ...
        public int UpdateOtherProcessingBillingMethod(int AppId, string billingMethod, bool bAssessments, string RollingReserve)
        {
